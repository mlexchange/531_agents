...
from typing import Any, Deque, Dict, Generator, Iterator, List, Optional, Union

# isort: off
from langgraph.checkpoint.memory import MemorySaver
from osprey.graph import create_graph  # noqa:I201 conflict isort flake8
...
from pydantic import BaseModel, Field  # noqa:I201 conflict isort flake8
...
_log_buffer: Deque[str] = deque(maxlen=1000)  # Keep last 1000 log entries
...
class Pipeline:
    ...
    def __init__(self):
        ...
        _log_buffer.append(
            ...
            f"Log capture active - buffer capacity: {_log_buffer.maxlen}"
        )
                    ...
                    if response:
                        ...
                        if len(response) > 50000:  # 50KB threshold for chunking
                            ...
                            for i in range(0, len(response), chunk_size):
                                chunk = response[i : i + chunk_size]  # noqa: E203
                                yield chunk
                        else:
                            yield response
                    else:
    ...
    def _convert_figure_to_static_url(
        ...
        try:
            ...
            # NOTE: Pipeline always runs in container where
            # PROJECT_ROOT/AGENT_DATA_DIR are set by deployment system
            ...
            if str(figure_path).startswith(host_agent_prefix):
                ...
                relative_path = str(figure_path)[len(host_agent_prefix) :]  # noqa E203
            elif str(figure_path).startswith(container_agent_prefix):
                # Container format (edge case)
                relative_path = str(figure_path)[len(container_agent_prefix) :]  # noqa E203
            else:
            ...
            return (
                f"{markdown_image}\n\n"
                f"*Source: {capability} | Created: {created_at_str} | "
                f"File: {Path(figure_path).name}*"
            )
    ...
    def _handle_log_command(self, command: str) -> Iterator[Union[str, dict]]:
        ...
        try:
            parts = command.split()
            # cmd = parts[0]  # /logs # it never used
    ...
    def _get_container_logs(self, lines: int = 100, container_name: str = "pipelines") -> str:
        ...
        try:
            ...
            if not _log_buffer:
                return (
                    "No logs captured yet. Log capture system is active and will "
                    "collect logs as they are generated."
                )
