...
class BL531DataAPI:
    ...
    def _categorize_data(self, key: str, data: np.ndarray, run_data: RunData):
        ...
        if "image" in key_lower:
            ...
            motor in key_lower for motor in ["motor", "hexapod", "angle", "mono", "_readback"]
        ):
    ...
    def _mock_run_data(self, run_uid: str) -> RunData:
        ...
        run_data.images = {"det_image": "Available (not loaded - use get_image() to load)"}
...
if MOCK_MODE:
    ...
    bl531_data = BL531DataAPI(tiled_uri="http://mock-tiled:8000", api_key="mock", mock_mode=True)
else:
    ...
    bl531_data = BL531DataAPI(tiled_uri=tiled_uri, api_key=api_key, mock_mode=False)
