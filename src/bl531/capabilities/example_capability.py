...
This is a complete example showing how to implement a capability in the Alpha Berkeley Framework.
...
# Registry for accessing context types
from osprey.registry import get_registry

# Context management for extracting inputs
from osprey.context.context_manager import ContextManager

# Import your context classes (defined in context_classes.py)
from bl531.context_classes import ExampleContext
...
logger = get_logger("example_capability")
...
class ExampleCapability(BaseCapability):
    ...
    provides = [
        # List the context types this capability creates/outputs
        # Must match CONTEXT_TYPE in your context classes
        "EXAMPLE_CONTEXT"
    ]
    """What context types does this capability CREATE and return?"""
    
    requires = [
        # List the context types this capability needs as input
        # These must be provided by other capabilities or user input
        # Example: registry.context_types.TIME_RANGE
    ]
    ...
    async def execute(state: AgentState, **kwargs) -> Dict[str, Any]:
        ...
        - Call your API / perform computations
        - Create output context objects
        - Store results back to state
        
        Args:
            state: Current agent state containing all context and execution plan
            **kwargs: Additional arguments (rarely used)
            
        Returns:
            Dict with updated state modifications
        ...
        # ──────────────────────────────────────────────────────────────────
        # 3.1: Get current step information
        # ──────────────────────────────────────────────────────────────────
        step = StateManager.get_current_step(state)
        logger.debug(f"Executing {__class__.__name__} for step: {step.get('context_key')}")
        
        # ──────────────────────────────────────────────────────────────────
        # 3.2: Setup streaming for user feedback (optional but recommended)
        # ──────────────────────────────────────────────────────────────────
        from osprey.utils.streaming import get_streamer
        streamer = get_streamer("example_capability", state)
        streamer.status("Starting example capability execution...")
        
        # ──────────────────────────────────────────────────────────────────
        # 3.3: Extract required inputs (if any)
        # ──────────────────────────────────────────────────────────────────
        # If your capability has requires=[], extract those contexts here
        
        # Example: Extract a required context
        # try:
        #     context_manager = ContextManager(state)
        #     contexts = context_manager.extract_from_step(
        #         step, state,
        #         constraints=["REQUIRED_CONTEXT_TYPE"],
        #         constraint_mode="hard"
        #     )
        #     required_input = contexts[registry.context_types.REQUIRED_CONTEXT_TYPE]
        ...
        # ──────────────────────────────────────────────────────────────────
        streamer.status("Example capability completed")
        ...
        return StateManager.store_context(
            state,
            "EXAMPLE_CONTEXT",  # Must match provides=[]
            step.get("context_key"),  # Storage key from execution plan
            output_context
        )
