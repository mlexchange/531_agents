...
class MoveCapability(BaseCapability):
    ...
    async def execute(state: AgentState, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("📊 Moving the motor now")
            # Move the motor
            result = bl531.scan(detectors=["diode"], motor=motor, start=target, stop=target, num=1)
            run_uid = result.run_uid

            logger.info(f"✅ motor movement completed. run_uid: {run_uid}")
            streamer.status("Verification now, retrieving data...")
    ...
    def _create_orchestrator_guide(self) -> Optional[OrchestratorGuide]:
        ...
        example1 = OrchestratorExample(
            step=PlannedStep(
                ...
                inputs=[{"MOTOR_NAME": "mono"}, {"TARGET_POSITION": "8979"}],
            ),
        ...
        example2 = OrchestratorExample(
            step=PlannedStep(
                ...
                inputs=[{"MOTOR_NAME": "gi_angle"}, {"TARGET_POSITION": "0.15"}],
            ),
        ...
        example3 = OrchestratorExample(
            step=PlannedStep(
                ...
                inputs=[{"MOTOR_NAME": "hexapod_motor_Tz"}, {"TARGET_POSITION": "2.5"}],
            ),
        ...
        example4 = OrchestratorExample(
            step=PlannedStep(
                ...
                inputs=[{"MOTOR_NAME": "mono"}, {"TARGET_POSITION": "10000"}],
            ),
    ...
    def _create_classifier_guide(self) -> Optional[TaskClassifierGuide]:
        ...
        return TaskClassifierGuide(
            ...
            examples=[
                ...
                ClassifierExample(
                    query="Set energy to 10 keV", result=True, reason="Move mono to target energy"
                ),
                ClassifierExample(
                    query="Move angle to 0.15", result=True, reason="Single position movement"
                ),
                ClassifierExample(
                    query="Set height to 2.5 mm", result=True, reason="Move to target height"
                ),
                ClassifierExample(
                    query="Go to iron K edge", result=True, reason="Move energy to specific edge"
                ),
                ClassifierExample(
                ...
                ClassifierExample(
                    query="Take an image", result=False, reason="Measurement only - use bl531_count"
                ),
            ],
