...
class CountCapability(BaseCapability):
    ...
    async def execute(state: AgentState, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Convert types
            num = int(num) if num is not None else 1  # or whatever default makes sense
            ...
            streamer.status("Measurement complete, retrieving data...")
    ...
    def _create_orchestrator_guide(self) -> Optional[OrchestratorGuide]:
        ...
        example1 = OrchestratorExample(
            step=PlannedStep(
                ...
                task_objective=(
                    "Measure beam intensity and return the value " "for display to user."
                ),
                expected_output=registry.context_types.RUN_DATA_CONTEXT,
                success_criteria=(
                    "Returns RUN_DATA_CONTEXT with detector values accessible "
                    "via get_summary()['beam_intensity'] or "
                    "['measurements']['diode']"
                ),
                inputs={
                    "DETECTORS": ["diode"],
            ...
            notes=(
                "Output: RUN_DATA_CONTEXT.get_summary() returns dict with "
                "'beam_intensity' and 'measurements' fields containing "
                "actual values"
            ),
        )
        ...
        example2 = OrchestratorExample(
            step=PlannedStep(
                ...
                expected_output=registry.context_types.RUN_DATA_CONTEXT,
                success_criteria=(
                    "Returns RUN_DATA_CONTEXT with image availability info "
                    "in get_summary()['available_images']"
                ),
                inputs={
                    "DETECTORS": ["det"],
                    "NUM_READINGS": "1",
            ...
            notes=(
                "Output: RUN_DATA_CONTEXT.get_summary() contains run_uid "
                "and available_images list"
            ),
        )
    ...
    def _create_classifier_guide(self) -> Optional[TaskClassifierGuide]:
        ...
        return TaskClassifierGuide(
            ...
            examples=[
                ...
                ClassifierExample(
                    query="Take an image", result=True, reason="Take image at current position"
                ),
                ClassifierExample(
                    query="Scan from 0.1 to 0.2", result=False, reason="Scanning - use bl531_scan"
                ),
            ],
