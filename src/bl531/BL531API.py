...
class BL531API:
    ...
    def count(
        self, detectors: List[str], num: int = 1, metadata: Optional[Dict[str, Any]] = None
    ) -> PlanResult:
        """
        ...
        plan_dict = {
            "item": {
                "name": "count",
                "kwargs": {"detectors": detectors, "num": num, "md": metadata or {}},
                "item_type": "plan",
            },
            "pos": "back",
        }
        ...
        return PlanResult(run_uid=run_uid, plan_name="count", timestamp=datetime.now())
    ...
    def scan(
        ...
        return PlanResult(run_uid=run_uid, plan_name="scan", timestamp=datetime.now())
    ...
    def automatic_gisaxs_alignment(self, metadata: Optional[Dict[str, Any]] = None) -> PlanResult:
        ...
        return PlanResult(
            run_uid=run_uid, plan_name="automatic_gisaxs_alignment", timestamp=datetime.now()
        )
    ...
    def automatic_diode_alignment(
        ...
        return PlanResult(
            run_uid=run_uid, plan_name="automatic_diode_alignment", timestamp=datetime.now()
        )
    ...
    def _validate_motor(self, motor: str):
        ...
        if motor not in BL531_MOTORS:
            raise ValueError(f"Invalid motor: {motor}. Available motors: {BL531_MOTORS}")
    ...
    def _mock_plan_execution(self, plan_name: str) -> PlanResult:
        ...
        return PlanResult(run_uid=mock_run_uid, plan_name=plan_name, timestamp=datetime.now())
    ...
    def _submit_plan(self, plan_dict: Dict[str, Any]) -> str:
        ...
        response = requests.post(
            f"{self.base_url}/api/queue/item/add", headers=self.headers, json=plan_dict
        )
...
if MOCK_MODE:
    ...
    bl531 = BL531API(
        base_url="http://mock-server:60610", api_key="mock", mock_mode=True  # Placeholder URL
    )
else:
    ...
    bl531 = BL531API(base_url=base_url, api_key=api_key, mock_mode=False)
