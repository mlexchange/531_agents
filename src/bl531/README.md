...
✓ **Complete example files** with inline documentation  
✓ **Clear guidance** on where to put your API calls, data structures, and logic  
...
bl531/
├── README.md                    # This file
├── config.yml                   # Configuration (API keys, models, etc.)
├── pyproject.toml              # Dependencies
├── requirements.txt            # Python packages
│
└── bl531/         # Your application code
    ├── __init__.py
    │
    ├── context_classes.py      # Define your data structures HERE
    │   └── Complete examples showing how to structure API responses,
    │       analysis results, and any data your capabilities work with
    │
    ├── registry.py             # Register your components HERE
    │   └── Simple configuration file that tells the framework
    │       what capabilities and context classes you've created
    │
    └── capabilities/           # Implement your logic HERE
        ├── __init__.py         # Overview and patterns
        └── example_capability.py.j2
            └── Complete template showing exactly how to:
                - Call external APIs
                - Process data
                - Handle errors
                - Return results
...
**👉 The file has complete examples showing different patterns!**
...
class MyAPICapability(BaseCapability):
    ...
    async def execute(state: AgentState, **kwargs):
        ...
        return StateManager.store_context(
            state, "MY_API_RESPONSE",
            step.get("context_key"), output
        )
```

**👉 The template has 200+ lines of documentation explaining every part!**

---

### 3. **Register Your Components**

Edit `bl531/registry.py`:
...
return extend_framework_registry(
    capabilities=[
        CapabilityRegistration(
            name="my_api",
            module_path="bl531.capabilities.my_api_capability",
...
**👉 The file explains all options including dependencies, data sources, and exclusions!**
...
**Example Flow**:
```
User Query → LLM Plans → Your Capability → API Call → Context Object → LLM Uses Data
```
...
**Example Flow**:
```
API Data → Your Analyzer → Analysis Results → LLM Generates Report
```
...
**Example Flow**:
```
User Query → Knowledge Retrieval → Documents/Data → LLM Answers with Context
```
...
**Example Flow**:
```
Step 1: Fetch Data → DATA_CONTEXT
Step 2: Process Data (requires DATA_CONTEXT) → PROCESSED_CONTEXT  
Step 3: Analyze (requires PROCESSED_CONTEXT) → ANALYSIS_CONTEXT
Step 4: Generate Report (requires ANALYSIS_CONTEXT) → Final Response
```
...
vim bl531/context_classes.py

# 2. Implement capability
vim bl531/capabilities/my_capability.py

# 3. Register components  
vim bl531/registry.py
...
**Problem**: "Module bl531.capabilities.X not found"  
...
- [ ] Sketch the data flow (User → Capability → Context → LLM)
...
✓ **Task Classification**: LLM determines which capabilities to use  
✓ **Orchestration**: LLM plans multi-step execution  
✓ **Memory**: Short and long-term context management  
✓ **Python Execution**: LLM can write and run analysis code  
...
from bl531.capabilities.my_capability import MyCapability
...
Start by opening `bl531/context_classes.py` and `bl531/capabilities/example_capability.py.j2` to see complete, documented examples of exactly what to do.
