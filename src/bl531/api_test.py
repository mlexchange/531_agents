...
try:
    result = bl531.scan(detectors=["diode"], motor="hexapod_motor_Ty", start=0, stop=0.3, num=5)
    print("   ✅ Success!")
    print(f"   run_uid: {result.run_uid}")
except Exception as e:
    print(f"   ❌ Error: {e}")
...
# Test 4:
print("\n4️⃣  Testing non existent detector...")
try:
    result = bl531.scan(
        detectors=["non-existing"], motor="hexapod_motor_Ry", start=-0.5, stop=0.5, num=5
    )
    print("   ✅ Success!")
    print(f"   run_uid: {result.run_uid}")
except Exception as e:
...
try:
    result = bl531.scan(
        detectors=["diode", "det"], motor="hexapod_motor_Ry", start=-0.5, stop=0.5, num=5
    )
    print("   ✅ Success!")
    print(f"   run_uid: {result.run_uid}")
except Exception as e:
